...
/datum/component/edible/proc/examine(datum/source, mob/user, list/examine_list)
	...
	if(GLOB.Debug2)
		...
		for(var/datum/reagent/reagent as anything in owner.reagents.reagent_list)
			examine_list += span_notice("- [reagent.declent_ru(NOMINATIVE)] [reagent.volume]мл.: [round(reagent.purity * 100)]% чистота")
...
/datum/component/edible/proc/created_by_processing(datum/source, atom/original_atom, list/chosen_processing_option)
	...
	if(!HAS_TRAIT(this_food, TRAIT_FOOD_DONT_INHERIT_NAME_FROM_PROCESSED) && original_atom.name != initial(original_atom.name))
		this_food.ru_names_rename(ru_names_toml("ломтик", suffix = " [original_atom.declent_ru(GENITIVE)]", override_base = "ломтик [original_atom.name]"))
		this_food.name = "ломтик [original_atom.name]"
...
/datum/component/edible/proc/TryToEat(mob/living/eater, mob/living/feeder)
	...
	if(eater == feeder)//If you're eating it yourself.
		...
		if(IsFoodGone(owner, feeder))
			return
		log_combat(feeder, eater, "скормил", owner.reagents.get_reagent_log_string())
		eater.visible_message(
