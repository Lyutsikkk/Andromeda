...
/datum/unit_test/head_transplant/Run()
	...
	alice.set_hairstyle("Очень длинные волосы", update = FALSE)
	alice.set_facial_haircolor(COLOR_LIGHT_PINK, update = FALSE)
	alice.set_facial_hairstyle("Выбритый", update = TRUE)
	...
	bob.set_hairstyle("Короткие волосы", update = FALSE)
	bob.set_facial_haircolor(COLOR_LIGHT_BROWN, update = FALSE)
	bob.set_facial_hairstyle("Борода (Полная)", update = TRUE)
	...
	TEST_ASSERT_EQUAL(alice.hairstyle, "Короткие волосы", "Bob's head was transplanted onto Alice's body, but their hairstyle is not Short Hair")
	...
	TEST_ASSERT_EQUAL(alice.facial_hairstyle, "Борода (Полная)", "Bob's head was transplanted onto Alice's body, but their facial hairstyle is not Beard (Full)")
