...
/proc/generate_chemwiki_line(datum/reagent/reagent, datum/chemical_reaction/reaction)
	...
	var/outstring = "!style='background-color:#FFEE88;'|{{anchor|[reagent.declent_ru(NOMINATIVE)]}}[reagent.declent_ru(NOMINATIVE)]<span style=\"color:[reagent.color];background-color:white\">▮</span>"
	...
	if(istype(reagent, /datum/reagent/inverse))
		...
		if(inverse_reagent)
			outstring += "\n<br>Inverse: \[\[#[inverse_reagent.declent_ru(NOMINATIVE)]|[inverse_reagent.declent_ru(NOMINATIVE)]\]\] <[reagent.inverse_chem_val*100]%"
	var/ph_color
	...
	//|{{RecursiveChem/Oil}}
	if(reaction)
		outstring += "{{RecursiveChem/[reagent.declent_ru(NOMINATIVE)]}}"
		outstring += "\n|"
